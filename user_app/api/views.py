...
from user_app.api.serializers import CollegeSerializer, DepartmentSerializer, EventSerializer, LeaveApplicationSerailizer, PasswordConfirmSerializer, PasswordResetSerializer, ProfileSerializer, RoleSerializer, UserCreateSerializer, GroupSerializer,LoginSerializer
from user_app.api.twilo import send_msg
from user_app.models import College, Department, Events, LeaveApplication, Profile, Roles
...
class LeaveApplicationViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if user_role == 'hod':
            serializer.validated_data['approved_hod'] = True
            serializer.validated_data['approved_principal'] = None
            serializer.validated_data['approved_AO'] = None
            serializer.validated_data['approved_director'] = None
            serializer.validated_data['approved_ceo'] = None
        elif user_role == 'principal':
            serializer.validated_data['approved_principal'] = True
            serializer.validated_data['approved_hod'] = None
            serializer.validated_data['approved_AO'] = None
            serializer.validated_data['approved_director'] = None
            serializer.validated_data['approved_ceo'] = None
        elif user_role == 'ao':
            ...
            serializer.validated_data['approved_director'] = None
            serializer.validated_data['approved_ceo'] = None
        serializer.save(applicant=profile, application_id=application_id)
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if user_role == 'hod':
            ...
            if action_taken == 'Approved':        
                instance.approved_principal = True
                instance.principal_approval_timestamp = timezone.now()
            elif action_taken == 'Rejected':
                instance.approved_principal = False
                instance.approval_status = 'Rejected'
            instance.save()
            send_email(sub=sub, msg=msg, to=to)
            send_msg(instance.applicant.phone, msg=f"{sub} {msg}")
            ...
            if action_taken == 'Approve':
                instance.approved_AO = True
                instance.AO_approval_timestamp = timezone.now()
            elif action_taken == 'Rejected':
                instance.approved_AO = False
            send_email(sub=sub, msg=msg, to=to)
            send_msg(instance.applicant.phone, msg=f"{sub} {msg}")
            return Response({"message": "updated successfully"}, status=status.HTTP_200_OK)
        else:
    ...
    def get_queryset(self):
        ...
        print(user_role)
        if user_role == 'hod':
            queryset = LeaveApplication.objects.filter(applicant__department=user.profile.department, approved_hod=None)

        elif user_role == 'principal':
            queryset = LeaveApplication.objects.filter(
                    applicant__college=user.profile.college,
                    approved_principal=None,  # Not yet approved by Principal
                    ).exclude(
                    Q(applicant__staff_role__role_type='Teaching') & ~Q(approved_hod=True)
                    )


        elif user_role == 'ao':
            queryset = LeaveApplication.objects.filter(applicant__staff_role__role_type__in=["Non-teaching", "Institution-staff"]).exclude(
                    Q(applicant__staff_role__role_type='Non-teaching') & ~Q(approved_pricipal=True)
                    )

        elif user_role == 'director':
             queryset = LeaveApplication.objects.all().exclude(
                    Q(applicant__staff_role__role_type__in=['Director', 'AO', 'CFO'])
                    )
        elif user_role == 'ceo':
             queryset = LeaveApplication.objects.filter(
                Q(applicant__staff_role__role_type__in=['Director', 'Principal', 'AO', 'CFO']) &
                Q(approved_ceo=None)  # Not yet approved by CEO
                ).exclude(Q(applicant__staff_role__role_type='Principal') & ~Q(approved_director=True))
        else:
            # Default case (e.g., regular staff)
            # You might want to handle this differently based on your requirements
